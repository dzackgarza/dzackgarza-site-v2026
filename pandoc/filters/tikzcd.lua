...
local function run_pdflatex_and_convert(tex_source, tmp_prefix, hash, doc_dir)
  ...
  local log_path = tmp .. "/pdflatex.log"
  local cmd1 = inputs_env .. "pdflatex -interaction=nonstopmode -output-directory=" .. tmp .. " " .. tex_path .. " > " .. log_path .. " 2>&1"
  local ok1 = os.execute(cmd1)
  if not ok1 then
    local log_file = io.open(log_path, "r")
    if log_file then
      io.stderr:write(log_file:read("*a"))
      log_file:close()
    end
...
local function make_html_output(svg_path, css_class)
  ...
  local html = '<div class="tikzcd-wrap" style="text-align:center;">'
