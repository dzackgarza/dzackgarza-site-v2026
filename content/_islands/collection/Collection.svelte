<script lang="ts">
  ...
  // index that folds Writing + Talks. Search + category facet + tag facet, plus
  // per-item tag chips and entry separators so entries read distinctly (the
  // presentation parity with the blog listing). Restrained per the site's
  // aesthetic (no shadows/badges/avatars).
  interface Item {
    title: string;
    category: string;
    description: string;
  ...
  function itemTags(i: Item): string[] {
    return i.tags === undefined ? [] : i.tags;
  }

  let categories = $derived([...new Set(items.map((i) => i.category).filter((c) => c))].sort());
  let allTags = $derived([...new Set(items.flatMap((i) => itemTags(i)))].sort());

  let filtered = $derived(
    items.filter((i) => {
      let matchesCategory = activeCategory === null || i.category === activeCategory;
      let matchesTag = activeTag === null || itemTags(i).includes(activeTag);
  ...
  // Item titles/descriptions can contain math; re-typeset whenever the rendered
  // set changes (hydration, search, facet clicks).
  $effect(() => {
    filtered;
...
{:else}
  <ul class="collection__list" bind:this={listEl}>
    {#each filtered as item}
      <li class="collection__item" class:collection__item--thumbed={item.image} data-tags={itemTags(item).join(",")}>
        {#if item.image}
          <img
        ...
        <div class="collection__body">
          ...
          {#if itemTags(item).length > 0}
            <span class="collection__item-terms">
              {#each itemTags(item) as tag}
                <button
                  type="button"
                  class="collection__item-tag"
                  class:is-active={activeTag === tag}
